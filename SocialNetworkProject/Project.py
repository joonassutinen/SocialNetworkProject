...
def GirvanNewman(G):
    ...
    for i in communitylist:
        ...
        communitysize.append(len(i))
    fig = go.Figure(data=[go.Table(columnwidth = [100,50], header=dict(values=["Community members", "Community size"]) ,cells=dict(values=[communitylist, communitysize
                     ], align=['left', "center"]))])
...
def CommunityRep(communityandsize, ListOfPosts):
    ...
    for i in communityandsize:
        communitylist.append(i[0])
        communitysize.append(i[1])
        ...
        communityrep.append(currentcommunityrep)
        print("Community:", i[0], "Size:", i[1], "Total Reputation:", i[2])
    fig = go.Figure(data=[go.Table(columnwidth = [100,50], header=dict(values=["Community members", "Community size", "Community reputation"]) ,cells=dict(values=[communitylist, communitysize, communityrep
...
def main():
    ...
    count_values = sorted((i for i in count_values if i < 200), reverse = True)
    print(count_values)
    ...
    xlist = []
    ylist = []
    for i, j in enumerate(count_values):
        if i == 0:
            xlist.append(0.000000001)
        elif i != 0:
            xlist.append(i)
    ...
    plt.plot(xlist, func_powerlaw(xlist, *popt), 'g-', label='power law')
    plt.plot(xlist,  ylist, 'bo', label='data')
    plt.title("power law distribution fitted to number of location occurences")
